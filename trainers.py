    ...
    for i, batch in enumerate(train_loader):
        ...
        if m_configs["layerless"]:
            if m_configs["endcaps"]:
                e_bidir = torch.cat(
                    [
                        batch.true_edges.to(device),
                        torch.stack(
                            [batch.true_edges[1], batch.true_edges[0]], axis=1
                        ).T.to(device),
                    ],
                    axis=-1,
                )
                ...
                e_bidir = torch.cat(
                    [
                        ...
                        torch.stack(
                            [
                                batch.layerless_true_edges[1],
                                batch.layerless_true_edges[0],
                            ],
                            axis=1,
                        ).T.to(device),
                    ],
            ...
            e_bidir = torch.cat(
                [
                    batch.layerwise_true_edges.to(device),
                    torch.stack(
                        [batch.layerwise_true_edges[1], batch.layerwise_true_edges[0]],
                        axis=1,
                    ).T.to(device),
                ],
                axis=-1,
        ...
        e_spatial = (
            torch.from_numpy(np.vstack([e_final.row, e_final.col])).long().to(device)
        )
        ...
        y_cluster = np.concatenate(
            [y_cluster.astype(int), np.ones(e_bidir.shape[1] * m_configs["weight"])]
        )
        ...
        loss = torch.nn.functional.hinge_embedding_loss(
            d, hinge, margin=m_configs["margin"], reduction=m_configs["reduction"]
        )
...
def train_embgnn(model, train_loader, optimizer, loss_fn, m_configs, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        fake_list = candidates[
            :,
            (batch.pid[candidates[0]] != batch.pid[candidates[1]])
            & ~(
        ...
        e_spatial = torch.cat(
            [
                fake_list,
                batch.true_edges.T.to(device),
                torch.stack(
                    [batch.true_edges[:, 1], batch.true_edges[:, 0]], axis=0
                ).to(device),
            ],
            axis=-1,
        )
        ...
        loss_1 = F.binary_cross_entropy_with_logits(
            pred.float(), y_edge.float(), pos_weight=torch.tensor(m_configs["weight"])
        )
...
def evaluate_adjacent(model, test_loader, loss_fn, m_configs):
    ...
    return (
        ...
        cluster_eff,
        total_loss,
        total_av_adjacent_nhood_size / (i + 1),
    )
...
def train_gnn(model, gnn_train_loader, optimizer, m_configs):
    ...
    for i, batch in enumerate(gnn_train_loader):
        ...
        loss = F.binary_cross_entropy_with_logits(
            pred.float(), y.float(), pos_weight=torch.tensor(m_configs["weight"])
        )
...
def evaluate_gnn(model, gnn_test_loader, m_configs):
    correct, total_positive, total_true, true_positive, total, total_loss = (
        0,
        1,
        0,
        0,
...
def train_all_emb(model, train_loader, optimizer, m_configs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # Concatenate all candidates
        e_spatial = torch.cat(
            [fake_list, torch.from_numpy(true_edges.T).to(device)], axis=-1
        )
