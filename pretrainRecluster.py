...
#     print("Preparing",event_file)
    X, pid, layers, true_edges = build_event(event_file, pt_min, feature_scale, adjacent)
...
#     print(args)
    ...
    train_dataset = [prepare_event(event_file, args.pt_cut, [1000, np.pi, 1000], args.adjacent) for event_file in all_events[:args.train_size]]
    test_dataset = [prepare_event(event_file, args.pt_cut, [1000, np.pi, 1000], args.adjacent) for event_file in all_events[-args.val_size:]]
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True)
    
    # Model config    
    e_configs = {"in_channels": 3, "emb_hidden": args.emb_hidden, "nb_layer": args.nb_layer, "emb_dim": args.emb_dim}
    m_configs = {"in_channels": 3, "emb_hidden": args.emb_hidden, "nb_layer": args.nb_layer, "emb_dim": args.emb_dim, "r": args.r_val, "hidden_dim": args.hidden_dim, "n_graph_iters": args.n_graph_iters}
    other_configs = {"weight": args.weight, "r_train": args.r_train, "r_val": args.r_val, "margin": args.margin, 'reduction':'mean'}
    ...
    embedding_optimizer = torch.optim.Adam(embedding_model.parameters(), lr=0.0005, weight_decay=1e-3, amsgrad=True)
    
    for epoch in range(args.pretrain_epochs):
        tic = tt()
        embedding_model.train()
        cluster_pur, train_loss = train_emb(embedding_model, train_loader, embedding_optimizer, other_configs)

        embedding_model.eval()
        with torch.no_grad():
            cluster_pur, cluster_eff, val_loss, av_nhood_size = evaluate_emb(embedding_model, test_loader, other_configs)
        wandb.log({"val_loss": val_loss, "train_loss": train_loss, "cluster_pur": cluster_pur, "cluster_eff": cluster_eff, "av_nhood_size": av_nhood_size})

    # Create and train main model       
    model = getattr(models, args.model)(**m_configs, pretrained_model=embedding_model).to(device)
    ...
    {'params': chain(model.emb_network_1.parameters(), model.emb_network_2.parameters())},
    ...
    for epoch in range(50):
        ...
        model.train()
        if args.adjacent:
            edge_acc, cluster_pur, train_loss = balanced_adjacent_train(model, train_loader, optimizer, multi_loss, m_configs)
        else:
            edge_acc, cluster_pur, train_loss = balanced_train(model, train_loader, optimizer, multi_loss, m_configs)
#         print("Training loss:", train_loss) 

        model.eval()
        if args.adjacent:
            with torch.no_grad():
                edge_acc, edge_pur, edge_eff, cluster_pur, cluster_eff, val_loss, av_nhood_size = evaluate_adjacent(model, test_loader, multi_loss, m_configs)
        else:
            with torch.no_grad():
                edge_acc, edge_pur, edge_eff, cluster_pur, cluster_eff, val_loss, av_nhood_size = evaluate(model, test_loader, multi_loss, m_configs)
        scheduler.step()
        wandb.log({"val_loss": val_loss, "train_loss": train_loss, "edge_acc": edge_acc, "edge_pur": edge_pur, "edge_eff": edge_eff, "cluster_pur": cluster_pur, "cluster_eff": cluster_eff, "lr": scheduler._last_lr[0], "combined_performance": edge_eff*cluster_eff*edge_pur + cluster_pur, "combined_efficiency": edge_eff*cluster_eff*edge_pur, "noise_1": multi_loss.noise_params[0].item(), "noise_2": multi_loss.noise_params[1].item(), "av_nhood_size": av_nhood_size})

        save_model(epoch, model, optimizer, scheduler, cluster_eff, m_configs, 'EmbeddingToAGNN/'+model_name+'.tar')
