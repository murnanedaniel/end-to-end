...
class GNNBase(LightningModule):
    ...
    """
    The base class for most GNN edge classification models.

    This class abstracts out the training and validation behaviour of edge classification GNNs. Note: It is specific to edge classification, do to the use of particular loss functions and weighting for this goal. 
    
    Todo:
        * Abstract out the edge classification nature of this class, or at least rename to goal-specific system, e.g. EdgeGNNBase, NodeGNNBase, etc.
    ...
    def training_step(self, batch, batch_idx):
        ...
        if "pid" in self.hparams["regime"]:
            ...
            loss = F.binary_cross_entropy_with_logits(output, y_pid.float())
        else:
            loss = F.binary_cross_entropy_with_logits(
                output, batch.y.float(), weight=manual_weights, pos_weight=weight
            )
