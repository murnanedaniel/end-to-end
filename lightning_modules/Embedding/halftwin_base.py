...
# Local Imports
from ..utils import graph_intersection, split_datasets, load_processed_dataset, build_edges
...
#         scheduler = [torch.optim.lr_scheduler.StepLR(optimizer[0], step_size=1, gamma=0.3)]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if 'ci' in self.hparams["regime"]:
            source_spatial, target_spatial = self(torch.cat([batch.cell_data, batch.x], axis=-1))
        else:
        ...
        if 'hnm' in self.hparams["regime"]:
            e_spatial = torch.cat([e_spatial, build_edges(source_spatial, self.hparams["r_train"], self.hparams["knn"], target_spatial=target_spatial)], axis=-1)
    ...
    def shared_evaluation(self, batch, batch_idx, knn_radius, knn_num, log=False):
        ...
        e_spatial = build_edges(source_spatial, knn_radius, knn_num, target_spatial=target_spatial)
