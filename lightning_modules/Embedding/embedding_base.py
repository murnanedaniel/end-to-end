...
class TripletEmbeddingBase(EmbeddingBase):
    ...
    def training_step(self, batch, batch_idx):
        ...
        triplet_loss = torch.nn.functional.triplet_margin_loss(
            anchors, positives, negatives, margin=self.hparams["margin"], p=1, swap=True
        )
        ...
        doublet_loss = torch.nn.functional.hinge_embedding_loss(
            d, hinge, margin=self.hparams["margin"], reduction="mean"
        ) / ((self.trainer.global_step + 100) / 100)
    ...
    def mine_triplets(self, batch, true_edges, spatial, r_max, k_max):
        ...
        squished_I = torch.cat(
            [
                ...
                * torch.ones(
                    squished_I.shape[0],
                    max(0, num_true_torch.max() - k_max),
                    dtype=int,
                    device=device,
                ),
            ],
        ...
        # ----------- Build Triplets
        selected_negatives = squished_I[pos_available][
            num_true_torch[pos_available, None]
            > torch.arange(squished_I.shape[1], device=device)
        ]
        triplets = torch.cat(
            [sorted_true_edges, selected_negatives.unsqueeze(0)], axis=0
        )
