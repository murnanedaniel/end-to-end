...
# Local Imports
from ..utils import graph_intersection, split_datasets, build_edges, load_processed_dataset
device = 'cuda' if torch.cuda.is_available() else 'cpu'

class TightFilterEdgeEmbeddingBase(LightningModule):
...
#         scheduler = [torch.optim.lr_scheduler.StepLR(optimizer[0], step_size=1, gamma=0.3)]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if 'ci' in self.hparams["regime"]:
            ...
            doublet_features = torch.cat([hit_features[batch.edge_index[0]], hit_features[batch.edge_index[1]]], axis=-1)
        else:
            doublet_features = torch.cat([batch.x[batch.edge_index[0]], batch.x[batch.edge_index[1]]], axis=-1)
        
        doublet_latent, doublet_score = self(doublet_features)
        ...
        if 'rp' in self.hparams["regime"]:
            ...
            triplet_edges = torch.cat([triplet_edges, torch.randint(0, batch.edge_index.shape[1], (2, n_random), device=self.device)], axis=-1)

        # Append Hard Negative Mining (hnm) with KNN graph
        if 'hnm' in self.hparams["regime"]:
            triplet_edges = torch.cat([triplet_edges, build_edges(doublet_latent, self.hparams["r_train"], self.hparams["knn"])], axis=-1)
        ...
        y_triplet = y_doublet[triplet_edges[0]] * (pid_doublet[triplet_edges[0]] == pid_doublet[triplet_edges[1]])

        # Append all truth
        if 'pid' not in self.hparams["regime"]:
            y_triplet = self.adjacent_triplet_truth(batch, y_triplet, triplet_edges)
            
        if 'noisy' in self.hparams["regime"]:
        ...
        y_triplet = torch.cat([y_triplet, torch.ones(truth_graph.shape[1], device=self.device)])
        ...
        if ('pid' in self.hparams["regime"]):
            ...
            filter_loss = F.binary_cross_entropy_with_logits(doublet_score.squeeze(), y_pid.float(), pos_weight = torch.tensor(self.hparams["weight"]))
        else:
            filter_loss = F.binary_cross_entropy_with_logits(doublet_score.squeeze(), batch.y.float(), pos_weight = torch.tensor(self.hparams["weight"]))
            
        self.log_dict({"emb_loss": emb_loss, "filter_loss": filter_loss})
        
        loss = emb_loss + filter_loss
    ...
    def shared_evaluation(self, batch, batch_idx, knn_radius, knn_num, log=False):
        ...
        doublet_latent, doublet_score = self(doublet_features)
        edge_prediction = (F.sigmoid(doublet_score) > self.hparams["filter_cut"]).squeeze()
        ...
        if 'pid' not in self.hparams["regime"]:
            y_triplet, triplet_edges = self.adjacent_triplet_truth(batch, y_triplet, triplet_edges, mask=True)
        ...
        if ('pid' in self.hparams["regime"]):
            filter_truth_graph = (batch.pid[batch.edge_index[0]] == batch.pid[batch.edge_index[1]])
        else:
            filter_truth_graph = batch.y
        
        filter_true = filter_truth_graph.sum()
        filter_true_positive = (filter_truth_graph & edge_prediction).sum()
        filter_positive = edge_prediction.sum()        
        
        eff = cluster_true_positive / cluster_true
        pur = cluster_true_positive / cluster_positive
        
        filter_eff = filter_true_positive / filter_true
        filter_pur = filter_true_positive / filter_positive
        
        current_lr = self.optimizers().param_groups[0]['lr']
        if log:
            self.log_dict({'val_loss': loss, 'eff': eff, 'pur': pur, "filter_eff": filter_eff, "filter_pur": filter_pur, "current_lr": current_lr})
        logging.info("Efficiency: {}".format(eff))
        logging.info("Purity: {}".format(pur))
        logging.info(batch.event_file)
        
        return {"loss": loss, "preds": triplet_edges.cpu().numpy(), "truth": y_triplet.cpu().numpy(), "truth_graph": truth_graph.cpu().numpy(), 'doublet_truth': y_doublet.cpu().numpy(), "filter_eff": filter_eff, "filter_pur": filter_pur}
    ...
    def get_true_edge_number(self, batch):
        ...
        true_doublets = batch.edge_index[0, batch.pid[batch.edge_index[0]] == batch.pid[batch.edge_index[1]]]
    ...
    def adjacent_triplet_truth(self, batch, y_triplet, triplet_edges, mask=False):
        ...
        duplicated_hid = (edge_i == edge_o).any(axis=0) + (edge_i[::-1] == edge_o).any(axis=0)
        ...
        if mask:
            ...
            triplet_edges = triplet_edges[:, torch.from_numpy(duplicated_hid).to(self.device)]
