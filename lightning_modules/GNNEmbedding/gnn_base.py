...
from ..utils import load_processed_dataset, build_edges, graph_intersection, MultiNoiseLoss
...
class GNNEmbeddingBase(LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = [torch.optim.AdamW([
            ...
            {'params': self.multi_loss.noise_params}], lr=(self.hparams["lr"]), betas=(0.9, 0.999), eps=1e-08, amsgrad=True)]
#         scheduler = [
#             {
...
#         ]
    ...
    def get_classification_loss(self, batch, doublet_score):
        ...
        edge_prediction = (F.sigmoid(doublet_score) > self.hparams["edge_cut"]).squeeze()
...
#         pass
    ...
    def shared_evaluation(self, batch, batch_idx, knn_radius, knn_num, log=False):
        ...
        emb_loss, truth_graph, y = self.get_embedding_loss(batch, latent_features, training=False, knn_num=knn_num, knn_radius=knn_radius)
        ...
        if log:
            self.log_dict({'val_loss': loss, 'eff': eff, 'pur': pur, "edge_eff": edge_eff, "edge_pur": edge_pur, "current_lr": current_lr})
    ...
    def get_embedding_loss(self, batch, latent_space, training=True, knn_num=100, knn_radius=1):
        ...
        if training:
            ...
            if 'rp' in self.hparams["regime"]:
                ...
                doublet_edges = torch.cat([doublet_edges, torch.randint(truth_graph.min(), truth_graph.max(), (2, n_random), device=self.device)], axis=-1)

            # Append Hard Negative Mining (hnm) with KNN graph
            if 'hnm' in self.hparams["regime"]:
                doublet_edges = torch.cat([doublet_edges, build_edges(latent_space, self.hparams["r_train"], self.hparams["knn"])], axis=-1)
        else:
        ...
        if training:
            doublet_edges = torch.cat([doublet_edges.to(self.device), truth_graph], axis=-1)
            y = torch.cat([y.to(self.device), torch.ones(truth_graph.shape[1], device=self.device)])
...
class GNNEdgeEmbeddingBase(GNNEmbeddingBase):
    ...
    def get_embedding_loss(self, batch, latent_space, training=True, knn_num=100, knn_radius=1):
        ...
        if training:
            ...
            # Append Hard Negative Mining (hnm) with KNN graph
            if 'hnm' in self.hparams["regime"]:
                triplet_edges = torch.cat([triplet_edges, build_edges(latent_space, self.hparams["r_train"], self.hparams["knn"])], axis=-1)
        ...
        if 'pid' not in self.hparams["regime"]:
            y_triplet, triplet_edges = self.adjacent_triplet_truth(batch, y_triplet, triplet_edges, mask=(~training))
