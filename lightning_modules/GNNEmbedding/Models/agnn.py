...
class NodeMeanNetwork(nn.Module):
    ...
    def forward(self, x, e, edge_index):
        ...
        node_inputs = torch.cat(
            [messages_in, x, node_mean.repeat((combined_message.shape[0], 1))], dim=-1
        )
        return self.network(node_inputs)
...
class AttentionNodeEmbedding(ToyGNNNodeEmbeddingBase):
    def __init__(self, hparams):
        ...
        self.embedding_network = make_mlp(
            hparams["hidden"],
            [hparams["hidden"]] * hparams["nb_emb_layer"] + [hparams["emb_dim"]],
            output_activation=None,
            layer_norm=False,
        )
...
class MeanAttentionNodeEmbedding(JetGNNNodeEmbeddingBase):
    def __init__(self, hparams):
        ...
        # Setup the node layers
        self.node_network = NodeMeanNetwork(
            hparams["hidden"],
            hparams["hidden"],
            hparams["nb_node_layer"],
            hparams["hidden_activation"],
...
class GlobalAttentionNodeEmbedding(JetGNNNodeEmbeddingBase):
    def __init__(self, hparams):
        ...
        # Setup embedding network
        self.embedding_network = make_mlp(
            hparams["hidden"] + hparams["in_channels"],
            [4 * hparams["hidden"]] * hparams["nb_emb_layer"] + [hparams["emb_dim"]],
            output_activation=None,
            layer_norm=False,
        )
...
class ConcatAttentionNodeEmbedding(JetGNNNodeEmbeddingBase):
    def __init__(self, hparams):
        ...
        # Setup embedding network
        self.embedding_network_1 = make_mlp(
            hparams["hidden"],
            [hparams["hidden"]] * hparams["nb_emb_layer"] + [hparams["emb_dim"]],
            output_activation=None,
            layer_norm=False,
        )
        self.embedding_network_2 = make_mlp(
            hparams["in_channels"],
            [4 * hparams["hidden"]] * hparams["nb_emb_layer"] + [hparams["emb_dim"]],
            output_activation=None,
            layer_norm=False,
    ...
    def forward(self, x, edge_index):
        ...
        # Shortcut connect the inputs onto the hidden representation
        x = torch.cat(
            [self.embedding_network_1(x), self.embedding_network_2(input_x)], dim=-1
        )
...
class ConcatPlusAttentionNodeEmbedding(JetGNNNodeEmbeddingBase):
    def __init__(self, hparams):
        ...
        self.embedding_network_3 = make_mlp(
            5 * hparams["hidden"],
            [5 * hparams["hidden"]] * 2 + [hparams["emb_dim"]],
            output_activation=None,
            layer_norm=False,
        )
