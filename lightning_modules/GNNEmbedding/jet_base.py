...
class ToyGNNEmbeddingBase(LightningModule):
    ...
    def setup(self, stage):
        if stage == "fit":
            ...
            self.min_edges = self.hparams["min_edges"] if "min_edges" in self.hparams else None
            ...
            self.trainset, self.valset, self.testset = [load_processed_dataset(input_dir, self.hparams["datatype_split"][i], min_edges=self.min_edges) for i, input_dir in enumerate(input_dirs)]
            self.multi_loss = MultiNoiseLoss(n_losses=2, device=self.device).to(self.device)
...
class JetGNNNodeEmbeddingBase(ToyGNNEmbeddingBase):
    ...
    def get_random_edges(self, truth_graph, subgraph):
        ...
        random_edges = torch.reshape(subgraph_hits, (2, subgraph_hits.shape[0]//2))[:, :n_random]
