...
class ToyGNNNodeEmbeddingBase(ToyGNNEmbeddingBase):
    ...
    def get_embedding_loss(self, batch, latent_space, training=True, knn_num=100, knn_radius=1):
        ...
        If validating:
        - get all pairs
        - trim to within r_val
        - get true positives
        - plug (all) into loss
        """
        # Construct training examples or KNN validation
        
        all_pairs = self.get_all_pairs(batch)
        
        reference = latent_space.index_select(0, all_pairs[1])
        neighbors = latent_space.index_select(0, all_pairs[0])
        d = torch.sum((reference - neighbors)**2, dim=-1)
                
        if training:
            selected_edges = np.arange(0, all_pairs.shape[1])
            
        else:
            selected_edges = d < self.hparams["r_val"]**2
            
        doublet_edges = all_pairs[:, selected_edges]
        
        truth_graph = all_pairs[:, batch.pid[all_pairs[0]] == batch.pid[all_pairs[1]]]
        ...
        d = d[selected_edges] * new_weights
