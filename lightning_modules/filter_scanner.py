...
class Filter_Model(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams["ratio"] != 0:
            ...
            fake_indices = torch.where(~batch.y.bool())[0][
                torch.randint(num_false, (num_true.item() * hparams["ratio"],))
            ]
            ...
            weight = (
                torch.tensor((~batch.y.bool()).sum() / batch.y.sum())
        ...
        output = (
            self(
                ...
                batch.e_radius[:, combined_indices],
                emb,
            ).squeeze()
        ...
        if "pid" in self.hparams["regime"]:
            y_pid = (
                batch.pid[batch.e_radius[0, combined_indices]]
                == batch.pid[batch.e_radius[1, combined_indices]]
            )
            loss = F.binary_cross_entropy_with_logits(
                output, y_pid.float(), pos_weight=weight
            )
        else:
            loss = F.binary_cross_entropy_with_logits(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        subset_ind = torch.randint(
            batch.e_radius.shape[1],
            (int(batch.e_radius.shape[1] * self.hparams["val_subset"]),),
        )

        output = (
            self(
                torch.cat([batch.cell_data, batch.x], axis=-1),
                batch.e_radius[:, subset_ind],
                emb,
            ).squeeze()
            if ("ci" in self.hparams["regime"])
            else self(batch.x, batch.e_radius[:, subset_ind], emb).squeeze()
        )
        ...
        if "pid" in self.hparams["regime"]:
            y_pid = (
                batch.pid[batch.e_radius[0, subset_ind]]
                == batch.pid[batch.e_radius[1, subset_ind]]
            )
            edge_true = y_pid.sum()
