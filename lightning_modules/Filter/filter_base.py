    ...
    def shared_evaluation(self, batch, batch_idx, log=False):
        ...
        for j in range(sections):
            ...
            if ('pid' not in self.hparams['regime']):
                ...
                val_loss = val_loss + F.binary_cross_entropy_with_logits(output, y_pid.float(), weight = manual_weights)
                            
        score_list = torch.cat(score_list)
        cut_list = score_list > self.hparams["val_filter_cut"]
...
#             self.logger.experiment.log({"roc" : wandb.plot.roc_curve( batch.y.cpu(), torch.stack([1 - score_list.cpu(), score_list.cpu()], axis=1))})
        ...
        return {"loss": val_loss, "true_positive": (batch.y.bool() & cut_list).float().cpu().numpy(), "true": batch.y.float().cpu().numpy(), "positive": cut_list.float().cpu().numpy()}
