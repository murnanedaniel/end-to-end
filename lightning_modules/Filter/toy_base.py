...
#         ]
    ...
    def shared_evaluation(self, batch, batch_idx, log=False):
        ...
        if log:
            self.log_dict({'edge_eff': edge_true_positive/edge_true, 'edge_pur': edge_true_positive/edge_positive, 'val_loss': val_loss, 'current_lr': current_lr})
        
        return {"loss": val_loss, "true_positive": (y_pid & cut_list).float().cpu().numpy(), "true": y_pid.float().cpu().numpy(), "positive": cut_list.float().cpu().numpy()}
...
#         print("Num true: {}, num fake: {}".format(true_indices.shape[0], fake_indices.shape[0]))
        ...
        # Shuffle indices:
        subgraph = bidir_edges[:, combined_indices[torch.randperm(len(combined_indices))]]
    ...
    def find_hard_negatives(self, bidir_edges, batch):
        ...
        with torch.no_grad():
            ...
            easy_indices = torch.where(easy_negatives)[0][torch.randperm(easy_negatives.sum())][:hard_negatives.sum()]
