...
#     train_events, val_events, test_events, _ = np.split(loaded_events, [sum(train_split[:x+1]) for x in range(0,len(train_split))])
...
def model_evaluation(model, trainer, fom= "eff", fixed_value=0.96):
...
def evaluate_set_root(r, model, trainer, goal=0.96, fom="eff"):
    eff, pur, _ = evaluate_set_metrics(r, model, trainer)
...
def get_metrics(test_results):
    f1s = [f1_score(result["truth"], np.ones(len(result["truth"]))) for result in test_results]
    mean_f1 = np.mean(f1s)
