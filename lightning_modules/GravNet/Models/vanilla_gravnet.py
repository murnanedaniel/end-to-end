...
class VanillaGravNet(GravNetBase):
    ...
    def __init__(self, hparams):
        ...
        self.space_encoder = make_mlp(hparams["in_channels"], [hparams["space_channels"]]*hparams["encoding_layers"],
        ...
        self.space_network = make_mlp(hparams["output_channels"], [hparams["space_channels"]]*hparams["encoding_layers"],
                                     output_activation=hparams["hidden_activation"],
                                     layer_norm=hparams["layernorm"])
        
        # Encode hidden features to next-block hidden features
        self.feature_network = make_mlp(hparams["output_channels"], [hparams["hidden_channels"]]*hparams["encoding_layers"],
                                     output_activation=hparams["hidden_activation"],
                                     layer_norm=hparams["layernorm"])
        
        
        # Decode hidden features to output features 
        self.decoder = make_mlp(2*hparams["hidden_channels"], [8*hparams["hidden_channels"]]*hparams["decoding_layers"] + [hparams["output_channels"]],
                                     hidden_activation=hparams["hidden_activation"], output_activation=None,
                                     layer_norm=hparams["layernorm"])
...
#         print("3:", torch.cuda.max_memory_allocated()/1024**3)
        ...
        weighted_sum_messages = scatter_add(hidden_features[start] * d_weight.unsqueeze(1), end, dim=0, dim_size=x.shape[0])
