...
class GravNetLite(GravNetBase):
    ...
    def __init__(self, hparams):
        ...
        self.space_encoder = make_mlp(hparams["hidden_channels"], [hparams["space_channels"]]*hparams["encoding_layers"],
                                     hidden_activation=hparams["hidden_activation"], output_activation=None,
                                     layer_norm=hparams["layernorm"])
        
        # Encode input features to hidden features
        self.feature_encoder = make_mlp(hparams["in_channels"], [hparams["hidden_channels"]]*hparams["encoding_layers"],
                                     hidden_activation=hparams["hidden_activation"], output_activation=hparams["hidden_activation"],
                                     layer_norm=hparams["layernorm"])
        
        # Encode hidden features to next-block hidden features
        self.feature_network = make_mlp(2*hparams["hidden_channels"], [hparams["hidden_channels"]]*hparams["encoding_layers"],
                                     output_activation=hparams["hidden_activation"],
                                     layer_norm=hparams["layernorm"])
        
        
        #Decode hidden features to output features 
        self.decoder = make_mlp((self.hparams["num_iterations"]+1)*hparams["space_channels"], [hparams["hidden_channels"]]*hparams["decoding_layers"] + [hparams["output_channels"]],
                                     hidden_activation=hparams["hidden_activation"], output_activation=None,
                                     layer_norm=hparams["layernorm"])
...
#                                      layer_norm=hparams["layernorm"])
    ...
    def forward(self, x):
        ...
        for i in range(self.hparams["num_iterations"]):
            ...
            weighted_sum_messages = scatter_add(features[start] * d_weight.unsqueeze(1), end, dim=0, dim_size=x.shape[0])
...
#         output = self.decoder(spatial)
    ...
    def get_truth(self, batch):
        ...
        truth = torch.cat([batch.layerless_true_edges, batch.layerless_true_edges.flip(0)], axis=-1)
    ...
    def get_true_predictions(self, batch, e_spatial, e_bidir):
        ...
        if "weighting" in self.hparams["regime"]:
            ...
            e_spatial, y, new_weights = graph_intersection(e_spatial, e_bidir, using_weights=True, weights_bidir=weights_bidir)
    ...
    def get_training_edges(self, truth, spatial, batch):
        ...
        if 'rp' in self.hparams["regime"]:
            ...
            e_spatial = torch.cat([e_spatial, torch.randint(truth.min(), truth.max(), (2, n_random), device=self.device)], axis=-1)

        # Append Hard Negative Mining (hnm) with KNN graph
        if 'hnm' in self.hparams["regime"]:
            e_spatial = torch.cat([e_spatial, build_edges(spatial, self.hparams["r_train"], self.hparams["knn"])], axis=-1)
                
        e_spatial, y, new_weights, weights_bidir = self.get_true_predictions(batch, e_spatial, truth)
        
        # Append all positive examples and their truth and weighting
        e_spatial = torch.cat([e_spatial.to(self.device), truth.transpose(0,1).repeat(1,1).view(-1, 2).transpose(0,1)], axis=-1)
        y = torch.cat([y.int(), torch.ones(truth.shape[1])])
        
        if "weighting" in self.hparams["regime"]:
            new_weights = torch.cat([new_weights, weights_bidir*self.hparams["weight"]])
        else:
            new_weights = torch.cat([new_weights, torch.ones(truth.shape[1], device=self.device) * self.hparams["weight"]])
    ...
    def hinge_loss_fn(self, spatial, e_spatial, y, new_weights):
        ...
        new_weights[y == 0] = 1 # Give negative examples a weight of 1 (note that there may still be TRUE examples that are weightless)        
        d = d * new_weights
    ...
    def grav_loss_fn(self, spatial, e_spatial, y, new_weights):
        ...
        loss = torch.nn.functional.binary_cross_entropy(d_weight, y.float().to(self.device))
