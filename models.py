...
class CheckMPNN_Network(nn.Module):
    ...
    def __init__(
        ...
        self.edge_classifier = make_mlp(
            2 * hidden_node_dim, [hidden_edge_dim, 1], output_activation=None
        )
...
class EdgeNetwork(nn.Module):
    ...
    def __init__(
        self, input_dim, hidden_dim=8, hidden_activation="Tanh", layer_norm=True
    ):
...
class NodeNetwork(nn.Module):
    ...
    def __init__(
        self, input_dim, output_dim, hidden_activation="Tanh", layer_norm=True
    ):
...
class BiEmbeddingToAGNN(nn.Module):
    ...
    def __init__(
        ...
        # Setup the edge network
        self.edge_network = EdgeNetwork(
            in_channels + hidden_dim + emb_dim,
            in_channels + hidden_dim,
            hidden_activation,
            layer_norm=layer_norm,
        )
    ...
    def forward(self, inputs, r, small_nhood=False):
        ...
        edge_index = edge_index[
            :, ((inputs.layers[edge_index[1]] - inputs.layers[edge_index[0]]) == 1)
        ]
        ...
        return (
            self.edge_network(x, edge_index),
            spatial,
            edge_index,
            len(edge_index[0]) / len(x),
        )
...
class EmbAGNN(nn.Module):
    ...
    def __init__(
        ...
        self.k = k
        # Setup the input network
        self.input_spatial_network = make_mlp(
            in_channels,
            [emb_dim],
            output_activation=hidden_activation,
            layer_norm=layer_norm,
        ...
        self.input_feature_network = make_mlp(
            in_channels,
            [hidden_dim],
            output_activation=hidden_activation,
            layer_norm=layer_norm,
        ...
        # Setup the edge network
        self.edge_network = EdgeNetwork(
            hidden_dim, hidden_dim, hidden_activation, layer_norm=layer_norm
        )
        # Setup the node layers
        self.node_network = NodeNetwork(
            hidden_dim, hidden_dim, hidden_activation, layer_norm=layer_norm
        ...
        # Setup gravnet
        self.emb_network = Embedding(
            emb_dim + hidden_dim + in_channels, emb_hidden, nb_layer, emb_dim
        )
    ...
    def forward(self, inputs):
        ...
        edge_index = radius_graph(
            spatial, r=self.r, batch=inputs.batch, loop=False, max_num_neighbors=30
        )
...
class EmbAGNNRecluster(nn.Module):
    ...
    def __init__(
        ...
        self.emb_network_2 = Embedding(
            in_channels + emb_dim + hidden_dim, emb_hidden, nb_layer, emb_dim
        )
    ...
    def forward(self, inputs):
        ...
        return (
            self.edge_network(features, edge_index),
            spatial,
            edge_index,
            len(edge_index[0]) / len(spatial),
        )
